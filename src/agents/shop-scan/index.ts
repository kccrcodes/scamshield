...
const SYSTEM_PROMPT = `You are ShopScan, ScamShield's e-commerce trust analyst for Vietnam.
Assess Shopee, TikTok Shop, Lazada, and marketplace seller profiles or listing text.

Rules:
- Return concise, concrete scam-risk signals, not legal conclusions.
- Focus on store-name mimicking, newly created accounts, all 5-star generic reviews, requests to contact outside the platform, prices 60% or more below market, no return policy, missing business registration, counterfeit indicators, and requests for bank transfer instead of platform payment.
- Treat copied brand names such as "Apple Offical Store" or "Samsung Vietnam Outlet" as possible impersonation when the evidence supports it.
...
function buildUserPrompt(input: AnalysisRequest) {
  return `Analyze the following marketplace seller profile or listing for a user in Vietnam.
...
export const shopScanAnalyzer: Analyzer = {
  id: "shop-scan",
  name: "ShopScan",
  description: "Checks seller profiles and listings for counterfeit and trust-risk signals.",
  supportedInputs: ["shop_profile", "listing_text", "url"],
  enabled: true,
